function [ xx,yy ] = generatePolygon( ctrX, ctrY, aveRadius_x, aveRadius_y , irregularity, spikeyness, numVerts, InRatio, NumPoly)
...
for i = 1:numVerts
    ...
    for j = 2:NumPoly+1
        x(i,j) = ctrX + (r_x(i)*(j-1)*InRatio)*cos(angle);
        y(i,j) = ctrY + (r_y(i)*(j-1)*InRatio)*sin(angle);
    end
end
for ii = 1:NumPoly+1
    x(numVerts+1,ii) = x(1,ii);
    y(numVerts+1,ii) = y(1,ii);
end
xx = x(:,2:end);
yy = y(:,2:end);
end
